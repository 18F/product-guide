 ...
 */
const Element = (props) => {
    const { annotations, decorate, decorations, element, renderAnnotation, renderDecoration, renderElement = (props) => React.createElement(CustomElement, Object.assign({}, props)), renderMark, selection, } = props;
    const ref = useRef(null);
    const editor = useEditor();
...
const Children = (props) => {
    ...
    for (let i = 0; i < node.nodes.length; i++) {
        ...
        if (Element$1.isElement(n)) {
            ...
            children.push(React.createElement(MemoizedText, { annotations: anns, decorations: decs, key: key.id, isLast: isLeafBlock && i === node.nodes.length, parent: node, renderAnnotation: renderAnnotation, renderDecoration: renderDecoration, renderMark: renderMark, text: n }));
        }
...
const Editor = (props) => {
    ...
    const onDOMSelectionChange = useCallback(debounce_1(() => {
    ...
    return (React.createElement(EditorContext.Provider, { value: editor },
        React.createElement(ReadOnlyContext.Provider, { value: readOnly },
            React.createElement(FocusedContext.Provider, { value: editor.isFocused() },
                React.createElement("div", Object.assign({ "data-gramm": false, role: readOnly ? undefined : props.role || 'textbox' }, attributes, { "data-slate-editor": true, "data-slate-node": "value", contentEditable: readOnly ? undefined : true, suppressContentEditableWarning: true, ref: ref, style: {
    ...
    function (_Editor) {
      ...
      createClass(ReactEditor, [{
        ...
        value: function getDecorations(node) {
          ...
          if (placeholder && Value.isValue(node) && node.nodes.length === 1 && Array.from(Node$1.texts(node)).length === 1 && Node$1.text(node) === '') {
